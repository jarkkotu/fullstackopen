...
const mostBlogs = (blogs) => {
  ...
  const authorsAndBlogs = _.map(Object.keys(groups), key => ({ author: key, blogs: groups[key].length }))
  return _.maxBy(authorsAndBlogs, o => o.blogs)
}

const mostLikes = (blogs) => {
  const groups = _.groupBy(blogs, blog => blog.author)
  const authorsAndLikes = _.map(Object.keys(groups), key => ({ author: key, likes: totalLikes(groups[key]) }))
